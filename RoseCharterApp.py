...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QPushButton, QLabel,
                             ...
                             QFileDialog, QFrame, QMessageBox, QDoubleSpinBox,
                             QInputDialog)
...
class RoseCharterApp(QMainWindow):
    def __init__(self):
        ...
        y += 10
        self.columnPromptLabel = QLabel('Coluna dos dados de direção ou sentido'
                                        ' (0-360):', self)
        ...
        self.mirrorCheckBox.setToolTip('Ao selecionar esta opção, sentidos '
                                       'opostos (ex: E e W) serão somados e '
                                       'espelhados.')
        ...
        self.directionsCombo.addItems(['Cardeais, colaterais e subcolaterais',
                                       'Cardeais e colaterais', 'Cardeais'])
    ...
    def open_file(self):
        ...
        if path != '':
                ...
                # Cria um dataframe a partir de um arquivo xlsx, xlsm ou ods
                    ...
                    if len(sheetNames) > 1:
                        sheet, ok = QInputDialog.getItem(
                            self, 'Selecionar aba', 'Planilha:', sheetNames
                        )
            ...
            # com o erro e esvazia as combo boxes
                ...
                self.filePromptLabel.setText('Não foi possível abrir o '
                                             'arquivo.')
    ...
    def select_column(self, col):
        try:
            ...
            if self.df[col].dropna().between(0, 360).all():
                self.column = col
                self.columnVerifyLabel.setText('OK! A coluna selecionada contém'
                                               ' apenas dados numéricos.')
                self.columnVerifyLabel.setStyleSheet('QLabel {color: green}')
                self.enable_widgets(True, True)
            else:
                self.columnVerifyLabel.setText('ATENÇÃO! Dados fora do '
                                               'intervalo permitido (0-360).')
                self.columnVerifyLabel.setStyleSheet('QLabel {color: red}')
                self.enable_widgets(True, False)
        except:
            self.columnVerifyLabel.setText('ATENÇÃO! A coluna selecionada '
    ...
    def build_diagram(self):
        ...
        try:
            ...
            # Cria 16 divisões de 22.5 graus para os pontos cardeais, colaterais
            # e subcolaterais
            bin_edges = numpy.arange(
                angle_start,
                361 + (angle_interval / 2),
                angle_interval
            )

            # Realiza a contagem de valores em cada uma das direções a partir
            # das divisões criadas
            ...
            if self.mirror == True:
                # Divide os dados em dois conjuntos (0-180 e 180-360), soma os
                # dois e duplica
                half = numpy.sum(numpy.split(number_of_strikes[:-1], 2), 0)
                two_halves = numpy.concatenate([half, half])
                ax.bar(
                    numpy.deg2rad(numpy.arange(0, 360, angle_interval)),
                    two_halves,
                    width=numpy.deg2rad(bar_width),
                    bottom=0.0,
                    edgecolor='white',
                    color=self.color
                )
            ...
            ax.set_thetagrids(
                numpy.arange(0, 360, angle_interval),
                labels=labels
            )
            ...
            msg = QMessageBox(parent=self, text='Não foi possível gerar o '
                                                'diagrama.\n\n' + str(e))
            msg.setWindowTitle("Erro")
            msg.setIcon(QMessageBox.Icon.Critical)
            msg.exec()
